...
export default function Calendar(props) {
  ...
  const startOfDay =
    Math.floor(current_date / ms_in_day) * ms_in_day - 19 * ms_in_hour; //The point was to use getTime, find the start of today, and find tasks for today, collab on it cuz it fell short
  ...
  return (
    //Attempting to format a grid with month full span, days Su-Sa under, extended columns under
    <Grid
      templateColumns='7'
    ...
    >
      ...
      {weekdays.map(weekday => (
        ...
        <Stack key={weekday} overflow='hidden'>
          <DayOfWeek
            bg={
              day.substr(0, 3) === weekday.substr(0, 3) ? 'cyan.300' : 'white'
            }
            color='black'
