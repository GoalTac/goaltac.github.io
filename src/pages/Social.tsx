...
export default function Social() {
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      if (data.user) setPost({ ...post, userid: data.user.id });
    };
    getUser();
  }, []);
      ...
      if (data) {
        ...
        if (likedError) {
          ...
          for (let x = 0; x < data.length; x++) {
            ...
            // set liked
            data[x].liked = likedData?.some(
              (liked) => liked.postid === data[x].id,
            );
  ...
  async function handleLike(
    ...
    if (checkError || checkData.length === 0) {
      ...
      if (error) console.log(error);
        ...
        setPosts(
          posts.map((post) =>
            post.id === id ? { ...post, liked: true } : post,
          ),
        );
      ...
      if (error) console.log(error);
        ...
        setPosts(
          posts.map((post) =>
            post.id === id ? { ...post, liked: false } : post,
          ),
        );
    }
    return undefined;
          ...
          <>
            {isShortScreen ? (
              ...
              <Flex direction="column" position="relative">
                ...
                <Box
                  p={4}
                  display={post.isHidden ? 'none' : 'block'}
                  position="absolute"
                  bottom={0}
                  left={0}
                  right={0}
                ...
                {post.isAd ? (
                  ...
                  <IconButton
                    ...
                    bg={post.liked ? 'pink.300' : 'white.300'}
                  />
                )}
                  ...
                  {post.isAd ? (
                    ...
                    <>
                      ...
                      <IconButton
                        ...
                        mr={2}
                        onClick={() => handleLike(post.id)}
                        bg={post.liked ? 'pink.500' : 'transparent'}
                      />
                    </>
                  )}
                </Flex>
            ...
            <DrawerBody p={0.5}>
              ...
              <FormControl mb={4} isRequired>
                ...
                <Input
                  ...
                  value={post.college}
                  onChange={(event) =>
                    setPost({ ...post, college: event.target.value })
                  }
                />
              </FormControl>
